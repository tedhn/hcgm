...
export default function EditCustomerPage() {
  ...
  const editCustomer = api.user.editCustomer.useMutation({
    onSuccess: async () => {
  ...
  useEffect(() => {
    if (data && !isLoading) {
      ...
      const mergedData = { ...initialData, ...cleanedData };
  ...
  if (isLoading || !data) {
    return <p className="px-4 py-6">Loading customer data...</p>;
  }
  ...
  return (
    <div className="w-full px-0 pt-4 lg:px-14">
      ...
      <h1 className="my-0 mb-10 text-3xl lg:my-10">Edit Customer</h1>
